...
def main():
    ...
    power_consumption = st.sidebar.slider("Power Consumption (kWh)", 0, 1000, 500)
    voltage = st.sidebar.slider("Voltage (V)", 110, 240, 220)
    frequency = st.sidebar.slider("Frequency (Hz)", 50, 60, 50)
    ...
    df = pd.DataFrame({
        ...
        'Power Consumption': np.random.randint(100, 1000, size=24),
        'Voltage': np.random.randint(110, 240, size=24),
        'Frequency': np.random.randint(50, 60, size=24)
    })

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Time'], y=df['Power Consumption'], mode='lines+markers', name='Power Consumption'))
